...
# set para
plotFigure = plotFig(chain_num = 1)
...
real_data = np.loadtxt("./real_data")
plotFigure.update_real_data(real_data)


thk_real = np.array([20,20,0]) 
vs_real = np.array([3.5,4,4.5])
...
for i in range(len(thk_real)):
    boundaries[i,0] = vs_real[i] - vs_real[i]*0.2
    boundaries[i,1] = vs_real[i] + vs_real[i]*0.2
...
plotFigure.savefig(figure,"best_modelq.jpeg")

figure = plotFigure.plot_best_fit()
plotFigure.savefig(figure,"best_syn_fitq.jpeg")

figure = plotFigure.plot_bestmodels_hit(np.array([2.5,5,100]),np.array([0,80,100]))
# np.array([1.8,4,100]) velocity 1.8km/s ~ 4km/s 100 points
plotFigure.savefig(figure,"best_model_histq.jpeg")

figure = plotFigure.plot_chain_misfit()
plotFigure.savefig(figure,"misfitq.jpeg")


figure = plotFigure.plot_best_fit_hist(np.array([-0.2,0.4,100]),np.array([2,3,100]),np.array([1.6,2.4,100]))
# np.array([-0.1,0.2,100])  amplitude of rf 
# np.array([2,3,100])       velocity of Rc
# np.array([1.6,2.4,100])    rg 
plotFigure.savefig(figure,"plot_best_fit_histq.jpeg")

figure = plotFigure.plot_var_statisc(boundaries, thk_real, vs_real)
plotFigure.savefig(figure,"staticq.jpeg")
